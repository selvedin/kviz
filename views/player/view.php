        ...
        <div v-if="isPlaying && !showResults" class='w-100 h-100'>
            ...
            <div class='quiz-part h-50 row'>
                <div v-if="question.question_type == 1" class="col-md-6 text-center">
                    <button @click="answerQuestion(0, '<?= __('False') ?>')" :disabled='!canAnswer' :class="[classObject, results.findIndex(r => r.q == question.id && r.a == 0) > -1 ? 'active':'btn-quiz-active']">
                        <?= __('False') ?>
                ...
                <div v-if="question.question_type == 1" class="col-md-6 text-center">
                    <button @click="answerQuestion(1, '<?= __('True') ?>')" :disabled='!canAnswer' :class="[classObject, results.findIndex(r => r.q == question.id && r.a == 1) > -1 ? 'active':'btn-quiz-active']">
                        <?= __('True') ?>
                ...
                <div v-if="[2,3].includes(+question.question_type)" v-for="(opt, index) in question.options" class="col-md-6 text-center">
                    <button @click="answerQuestion(opt.id, opt.content)" :disabled='!canAnswer' :class="[classObject, results.findIndex(r => r.q == question.id && r.a == opt.id) > -1 ? 'active':'btn-quiz-active']">
