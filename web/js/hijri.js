...
const toGregorianYear = y => {
  return Math.floor(y * 0.97 + 622);
};

const toHijriYear = y => {
  return Math.floor((y - 622) * 1.03);
};

const toGregorianCentury = y => {
  const year = toGregorianYear(y);
  if (year < 100) return 1;
  return Math.ceil(year / 100);
};

const toHijriCentury = y => {
  const year = toHijriYear(y);
  if (year < 100) return 1;
  return Math.ceil(year / 100);
};
