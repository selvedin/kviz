...
$(document).ready(function () {
  $("ul.pagination").addClass("pagination-round pagination-sm mt-5");
  $("ul.pagination li").addClass("page-item");
  $("ul.pagination li a").addClass("page-link");

  $(".toArabic").each(function (ind) {
    const text = $(this).text();
    $(this).text(numbersToArabic(text));
  });
});

const numbersToArabic = s =>
  s.replace(
    /\d/g,
    d => "\u0660\u0661\u0662\u0663\u0664\u0665\u0666\u0667\u0668\u0669"[d]
  );
...
function onTimesUp() {
  const el = document.getElementById("base-timer-path-remaining");
  el.classList.remove(COLOR_CODES.alert.color);
  el.classList.add(COLOR_CODES.info.color);
  clearInterval(timerInterval);
  mainApp.stopQuestion();
}
...
function setRemainingPathColor(timeLeft) {
  ...
  if (timeLeft <= Math.round((TIME_LIMIT / 100) * alert.threshold)) {
    el.classList.remove(warning.color);
    el.classList.add(alert.color);
  } else if (timeLeft <= Math.round((TIME_LIMIT / 100) * warning.threshold)) {
