...
<script>
    ...
    stop: function() {
      ...
      return new Promise(resolve => {
        ...
        audioRecorder.mediaRecorder.addEventListener("stop", () => {
          ...
          sendFileToServer(audioBlob).then(response => response.json())
            .then(data => {
              const responseElement = document.getElementById('response-transcript');
  ...
  async function sendFileToServer(blob) {
    ...
    formData.append('<?= $filename ?>', blob, '<?= $filename ?>.webm');
    formData.append('filename', '<?= $filename ?>');

    const response = await fetch(`<?= Url::to(['gpt/process-audio']) ?>`, {
      method: 'POST',
      body: formData
    });
